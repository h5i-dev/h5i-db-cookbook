...
# | 用語                  | 意味 |
# | ------------------- | --- |
# | EOD                 | エンド・オブ・デイ。価格・ポジション・マークの公式な日次締め |
# | スナップショット            | テーブルバージョンに名前とチェックサムを付けた固定。複製しないので O(1) |
# | 監査証跡                | いつ何が分かっていたかを、第三者が検証できる形で残した記録 |
# | `verify(deep=True)` | 保存された全バイトを再計算する、完全性の証明 |
# | `vacuum`            | 参照されていないストレージを回収する。バージョン履歴は消さない |
# | 保存期間（retention）     | 記録を破棄してよくなるまで、どれだけ保持する必要があるか |
#

...
# | 用語            | 意味 |
# | ------------- | --- |
# | インプライドボラティリティ | 価格モデルに入れるとオプションの市場価格が再現されるボラティリティ |
# | IV サーフェス      | 権利行使価格と満期にわたる IV を1つの面として捉えたもの |
# | チェーン          | ある時点における、1つの原資産の上場オプション全部 |
# | ATM           | アット・ザ・マネー。現在価格に近い権利行使価格 |
# | ターム構造         | 満期方向の IV の形。ここでは ATM で取る |
# | デルタ           | 原資産に対するオプション価格の感応度。「25デルタ」のように行使価格の目印にも使う |
# | リスクリバーサル      | 同じデルタのコールとプットの IV の差。スキューを1つの数で表す |
# | バタフライ         | 両翼と ATM の差。スマイルの曲がり具合を1つの数で表す |
# | テナー           | 満期までの残存期間。保存せず `expiry - ts` から必ず導出する |
# | マーク           | ポジションの公式価格。1日1回、あるいはもっと頻繁に付ける |
#

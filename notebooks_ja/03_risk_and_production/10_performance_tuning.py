...
# | 用語           | 意味 |
# | ------------ | --- |
# | 枝刈り（pruning） | 条件に合いようがない時間範囲のセグメントを、I/O の前にまるごと飛ばすこと |
# | プロジェクション     | クエリが必要とする列だけを読むこと。列指向ストレージだから安い |
# | コミットの粒度      | 1コミットに何行入れるか。小さなファイルがいくつできるかを決める |
# | `compact`    | 小さなセグメント群を大きく少数にまとめ、新しいバージョンとして書く |
# | セグメント        | ある時間範囲の行を持つ、イミュータブルな Parquet ファイル1つ |
# | リソースガード      | `timeout`、`max_rows`、`memory_limit`。共有マシンで行儀よくするための制限 |
#

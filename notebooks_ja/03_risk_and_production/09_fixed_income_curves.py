...
# | 用語            | 意味 |
# | ------------- | --- |
# | イールドカーブ       | 満期に対して金利をプロットしたもの |
# | パーカーブ         | 債券価格が100になるクーポンとして表したカーブ |
# | テナー           | カーブ上の満期の点。2年、10年など |
# | ロング形式         | テナーごとに列を作らず、`(日付, テナー, 利回り)` を1行にする持ち方 |
# | 傾き（slope）     | 長いテナーから短いテナーを引いた利回り差。10年マイナス2年など |
# | 曲率（curvature） | カーブの中央を両端と比べた値。バタフライとも呼ぶ |
# | キャリー          | カーブが動かないとして、時間が経つだけで得られる収益 |
# | ロールダウン        | 右上がりのカーブ上で債券が低い点へ移ることで得られる利益 |

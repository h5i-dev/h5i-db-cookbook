...
#
# ここでは意図して SQL 文字列だけを書きます。SQL そのものが主題だからです。クックブックの
# 他のレシピは遅延 DataFrame ビルダ（`db.table(...)` と動詞、レシピ09）を優先します。
# ビルダが吐くのはここに並ぶ文とまったく同じもので、組み立てたクエリに `.sql()` を呼べば
# 生成された SQL がそのまま表示されます。

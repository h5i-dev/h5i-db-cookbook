...
# | 用語           | 意味 |
# | ------------ | --- |
# | シグナル         | データから計算した、取るべきポジションを示す数値 |
# | モメンタム（12-1）  | 直近12か月から直近1か月を除いたリターンで上がったものを買う戦略 |
# | クロスセクション     | 同じ時点で資産どうしを順位づけること。自分の過去とは比べない |
# | リバランス        | 目標ウェイトを計算し直し、そこへ向けて定期的に売買すること |
# | バックテスト       | 過去データで戦略をシミュレーションすること |
# | ベーシスポイント（bp） | 1パーセントの100分の1。取引費用はこの単位で課す |
# | P&L          | 損益 |
# | スナップショット     | テーブルバージョンの名前付き固定。再実行が同じバイト列を読む |
#

...
# | 用語               | 意味 |
# | ---------------- | --- |
# | 共和分              | 個々には彷徨うのに、その差は彷徨わない2つの価格系列 |
# | Engle-Granger 検定 | その標準的な2段階検定。回帰の残差に対して行う |
# | ヘッジレシオ           | 第1レグ1単位に対して第2レグを何単位持つか |
# | スプレッド            | ここではヘッジ済みペアの残差。ビッド・アスク・スプレッドではない |
# | Zスコア             | スプレッドがローリング平均から標準偏差いくつ分離れているか |
# | 平均回帰             | 平均へ戻ろうとする性質。ペアトレードが賭けているのはこれ |
# | シャープレシオ          | 年率リターンを年率ボラティリティで割った値。リスク1単位あたりの報酬 |
# | ドローダウン           | エクイティカーブの直近ピークからの下落率 |
# | バージョンのピン留め       | 実行の入力を特定のバージョンに固定すること。再実行が同じバイト列を読む |
#

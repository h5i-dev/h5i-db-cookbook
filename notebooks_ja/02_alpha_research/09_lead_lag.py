...
# | 用語                  | 意味 |
# | ------------------- | --- |
# | リードラグ               | ある銘柄の動きが、別の銘柄の動きに系統的に先行すること |
# | 相互相関関数              | 2つの系列を、さまざまな時間ずれで相関させたもの |
# | 非同期（asynchronous）   | 2つの銘柄がそろわない別々の瞬間にティックを出すこと |
# | Epps 効果             | サンプリング間隔を短くするほど、測った相関がゼロへ落ちる現象 |
# | locf                | 直前の観測値の持ち越し。このバイアスを生む「対処法」の1つ |
# | Hayashi-Yoshida 推定量 | 重なり合うティック区間を直接組にする共分散推定量。グリッドが要らない |
# | ADR                 | 現地で取引される外国株。本国上場との組はリードラグの定番 |
#

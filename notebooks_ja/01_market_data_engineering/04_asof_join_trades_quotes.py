...
# | 直前気配            | 約定がプリントされた瞬間に有効だった気配 |
# | ミッド             | ビッドとアスクの平均。「価格」を1つの数で表すときの標準的な代用値 |
# | クォーテッド・スプレッド    | 約定前の、表示されたままのアスクとビッドの差 |
# | 実効スプレッド         | 約定価格と直前ミッドとの距離を2倍した値 |
# | 実現スプレッド         | 同じものを数分後のミッドに対して測った値。価格の逆行分を切り分ける |
# | 約定の符号付け         | 買い方と売り方のどちらが仕掛けたかを推定すること |
# | Lee-Ready 法     | その標準ルール。約定価格を直前ミッドと比べる |
# | ティックテスト         | 代替ルール。直前の約定価格と比べる |
# | 許容時間（tolerance） | 気配の最大許容経過時間。古すぎる気配は結合せずに拒否される |
#

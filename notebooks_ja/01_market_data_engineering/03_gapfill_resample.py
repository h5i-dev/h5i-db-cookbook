...
# | 用語                 | 意味 |
# | ------------------ | --- |
# | 非流動的（illiquid）     | 売買が少なく、多くの区間にそもそも約定がない状態 |
# | gapfill / resample | 不規則な系列を規則的な時間グリッドに載せる |
# | locf               | 直前の観測値の持ち越し。3つの補完方針のうちの1つ |
# | interpolate        | 穴の前後2点を直線で結ぶ補完方針 |
# | 幻のリターン             | 持ち越した同じ価格から計算されるリターン。誰も取引できなかったもの |
# | 共分散行列              | リターンの共分散。ほとんどのポートフォリオ・リスクモデルの入力になる |
#

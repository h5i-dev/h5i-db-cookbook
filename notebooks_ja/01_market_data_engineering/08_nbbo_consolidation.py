...
# | 用語         | 意味 |
# | ---------- | --- |
# | 取引所（venue） | 1つの取引所や取引システム。米国株は十数か所で同時に取引される |
# | 市場の分散      | 同じ銘柄が多数の取引所で取引され、どの板も市場全体ではない状態 |
# | ベストプライス    | 1つの取引所における最良ビッドと最良アスク、およびその数量 |
# | NBBO       | 全取引所を通じた最良気配。執行品質はこれを基準に測られる |
# | インサイド      | 今出ている最良の価格。その価格を出した取引所が「インサイドを作る」 |
# | ロック／クロス    | ビッドとアスクが等しい、またはビッドがアスクを上回る一時的な状態 |
